...
struct MoveNodesBatchAction: CanvasAction {
    let id = UUID()
    ...
    func apply(on model: AppModel) {
        for (i, id) in nodeIds.enumerated() {
            model.updatePositionInternal(nodeId: id, position: newPositions[i])
        }
    }
    
    func undo(on model: AppModel) {
        for (i, id) in nodeIds.enumerated() {
            model.updatePositionInternal(nodeId: id, position: oldPositions[i])
        }
    }
